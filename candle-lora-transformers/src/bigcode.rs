...
fn embedding(
    ...
    merge: bool,
    lora_config: LoraConfig,
) -> Result<CustomEmbedding> {
    let embeddings = vb.get((vocab_size, hidden_size), "weight")?;
    let mut this = CustomEmbedding {
        inner: Arc::new(Embedding::new(embeddings, hidden_size)),
    };
...
impl Block {
    fn load(vb: VarBuilder, cfg: &Config, merge: bool, lora_config: LoraConfig) -> Result<Self> {
        let hidden_size = cfg.hidden_size;
        let inner_dim = cfg.n_inner.unwrap_or(4 * hidden_size);
        let ln_1 = layer_norm(hidden_size, cfg.layer_norm_epsilon, vb.pp("ln_1"))?;
        let attn = Attention::load(vb.pp("attn"), cfg, merge, lora_config.clone())?;
        let ln_2 = layer_norm(hidden_size, cfg.layer_norm_epsilon, vb.pp("ln_2"))?;
        let mlp = Mlp::load(inner_dim, vb.pp("mlp"), cfg, merge, lora_config)?;
        Ok(Self {
            ln_1,
            attn,
            ln_2,
...
impl GPTBigCode {
    ...
    pub fn load(vb: VarBuilder, cfg: Config, merge: bool, lora_config: LoraConfig) -> Result<Self> {
        let hidden_size = cfg.hidden_size;
